/* 
 * File:   Maestro_Proyecto2.c
 * Author: Jose Pablo Petion        201151
 *         Sergio Alejandro Boch    20887
 *
 * Pic Maestro: Control de entradas analogicas, cambio de estados, control
 *              USART por medio de interfaz en computadoras
 * 
 * Created on May 16, 2022, 7:25 PM
 */
...
#define IN_MAX2 63                // Valor máximo de entrada del potenciometro (Para entradas de USART))
...
#define OUT_MAX2 127              // Valor máximo de entrada del potenciometro (Para contolar motores SPI))
#define OUT_MAX3 80               // Mapeo para garra a 45º
...
uint8_t LEC1, LEC2, LEC3, LEC4;                     //variables de reproducción
...
void __interrupt() isr (void){
    ...
    if(INTCONbits.RBIF){
        ...
        else if (!ACTION){                      //Boton de acción
        ...
        else if (!ACTION2){                 //Pulsador De guardado de segunda posición
            if (PORTDbits.RD0==1){
                write_EEPROM(5, var1);
                __delay_ms(50);
                write_EEPROM(6, var2);
                ...
                LEC1 = read_EEPROM(5);          //REPRODUCCIÓN 1
                SSPBUF = (uint8_t) (map(LEC1, IN_MIN, IN_MAX, OUT_MIN, OUT_MAX2));
                __delay_ms(50);
                LEC2 = read_EEPROM(6);          //REPRODUCCIÓN 2
                CCPR = map(LEC2, IN_MIN, IN_MAX, OUT_MIN, OUT_MAX);
                CCPR1L = (uint8_t)(CCPR>>2);
                CCP1CONbits.DC1B = CCPR & 0b11;
                __delay_ms(50);
        ...
        //COMUNICACIÓN EUSART
        if(PIR1bits.RCIF){                      //Comuniación con la interfaz en PC        
            
            if (PORTDbits.RD2){
            RECIV = 0b00111111 & RCREG;         //Verificación del valor a mapear
